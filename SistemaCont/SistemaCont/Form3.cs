    ...
    public partial class Form3 : Form
    {
        ...
        public void metodo()
        {
            if (cu != null & cu1 != null)
            {
                ...
                d.Rows.Add("Utilidad bruta", "", ef.sum(cu)-ef.sum(cu1));
                foreach (Cuenta cc in cu2)
                {
                    d.Rows.Add(cc.CUENTA, cc.TIPO, cc.SALDO);
                }
                d.Rows.Add("Total Gastos", "", ef.sum(cu2));
                Double imp= (ef.sum(cu) - ef.sum(cu1) - ef.sum(cu2));
                d.Rows.Add("Utilidad", "", imp);
                if (im.Text != "")
                {
                    Double imp2 = (imp * (Double.Parse(im.Text) / 100));
                    d.Rows.Add("Tasa de impuesto",im.Text,imp2);
                    d.Rows.Add("Utilidad despues de impuesto","",(imp-imp2));
                }
            }
